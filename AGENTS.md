...
- `rots_combat` (L3 static library stood up by the combat-seed wave: `skill_timer.cpp`/`battle_mage_handler.cpp`/`weapon_master_handler.cpp`/`wild_fighting_handler.cpp`, linked as `RotS::combat`, PUBLIC-linking only `RotS::entity`/`RotS::core`/`RotS::platform` — no L3-peer link, unlike `rots_world`) and its `CombatLayerAcyclicity` linkcheck are CI-linked the same way; it is the third and final L3 peer to exist as a build target. It is 4 of the spec's 16-TU sketch for the row (SEED-CLEAN per the combat-census) — `profs` (caveated) and the 11 DEFER TUs (`spec_ass`/`olog_hai`/`clerics`/`mage`/`mystic`/`mobact`/`spell_pa`/`limits`/`ranger`/`spec_pro`/`fight`) are deferred follow-on, blocked by the app-side `handler.cpp`/`utility.cpp` remainder plus (for the mob-AI/spec-proc TUs) a command-dispatch seam — see docs/BUILD.md "Library layering" and `.superpowers/sdd/combat-census.md`. The wave also completed the placement-seam wave's two remaining deferral riders: the time quartet (`real_time_passed`/`mud_time_passed`/`day_to_str`/`age`) rejoined `rots_core` (`consts.cpp`), and `NumberedName` extracted to `rots/platform/numbered_name.h` (L0 platform tier, not the plan's core sketch — see docs/BUILD.md's `rots_combat` section, "NumberedName's L0-visibility tier decision") so `parse_numbered_name`→`rots_util.cpp` and `get_char`→`entity_lifecycle.cpp` could complete; the poison-notification hook cluster (`obj_from_char`/`extract_obj`) remains backlog.
...
- The weather/room-arg OB/PB/DB trio that only `combat_manager` called
  ...
  `weather_data`/`room_data` arguments — **itself now splits across library tiers, as of the
  placement-seam wave (Task 5):** `get_real_dodge` moved into **`src/char_utils_combat.cpp`**
  (`rots_entity`, L2 lib) because it's entity-pure combat-stat arithmetic; `get_real_OB` and
  `get_real_parry` stay in **`src/utility.cpp`** (app-compiled) because both call into
  `player_spec::weapon_master_handler`, an app-tier spec-handler edge `get_real_dodge` doesn't
  share — each has a one-line comment at its definition explaining the split. `char_utils_combat.cpp`
  itself is still live: it also defines `get_engaged_characters`/`is_victim_player`/
  `get_controlling_player`/`on_attacked_character`, used by `fight.cpp`/`clerics.cpp`/`ranger.cpp`.
