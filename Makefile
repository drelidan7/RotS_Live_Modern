...
# rots_world_linkcheck / rots_combat_linkcheck must be built explicitly: the PlatformLayerAcyclicity /
# CoreLayerAcyclicity / EntityLayerAcyclicity / PersistLayerAcyclicity / WorldLayerAcyclicity /
# CombatLayerAcyclicity CTests execute their binaries,
...
# for rots_world_linkcheck, and combat-seed Task 1 for rots_combat_linkcheck).
test: $(CMAKE_CACHE)
	+$(CMAKE) -S $(SRC_DIR) -B $(BUILD_DIR)
	+$(CMAKE) --build $(BUILD_DIR) --target ageland ageland_tests rots_platform_linkcheck rots_core_linkcheck rots_entity_linkcheck rots_persist_linkcheck rots_world_linkcheck rots_combat_linkcheck -j16
