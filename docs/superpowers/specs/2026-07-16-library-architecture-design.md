...
| `rots_combat` | L3 | 16 | `fight, limits, skill_timer, mobact, ranger, clerics, mage, mystic, profs, spell_pa, spec_pro, spec_ass, battle_mage_handler, weapon_master_handler, wild_fighting_handler, olog_hai` (as-built: 13 of 16 original-sketch TUs have joined `rots_combat` — `skill_timer`/`battle_mage_handler`/`weapon_master_handler`/`wild_fighting_handler` (combat-seed wave), `clerics`/`fight` (combat-pilot wave, joint membership), `olog_hai`/`mystic`/`profs` (combat-trio wave, one membership commit — olog_hai and mystic each promoted **standalone**, closed over their own edges independently; `profs` rode as a census-gated rider whose one real coupling, `scale_guardian`/mystic.cpp, is a one-directional gate, not a cycle), `limits` (behavior wave, a standard L3 DEFER-row promotion), and `spell_pa`/`mage`/`ranger` (spell-family closure wave, ONE joint membership commit — the true bidirectional `spell_pa↔mage` cycle plus ranger's one-directional dependency on spell_pa forced all three into the same commit) — `profs`'s SEED-WITH-SEAM caveat is now RESOLVED and the remaining **2** (`mobact` excepted — see below — `spec_pro, spec_ass`) DEFER, all still app-compiled. **Addendum, 2026-07-22 (spec-pair wave): DEFER resolved to 0.** `spec_pro`/`spec_ass` did NOT join `rots_combat` either — like `mobact` below, both homed in `rots_script` instead (SEQUENTIAL commits, not joint; the driver-homes-with-engine ruling extended one link further), closing this row's DEFER count to zero; see that row below and §10's "spec-pair wave, step 4 twelfth slice" note. This is history, not a rewrite: the row's original 16-TU sketch genuinely counted `spec_pro`/`spec_ass` as `rots_combat` candidates at the time it was written; the twelfth slice is where that assumption was revisited and settled. The blocker-buster wave also added two further TUs NOT in this row's original 16-TU sketch — `combat_hooks.cpp`/`visibility.cpp`, enabler infrastructure rather than DEFER-TU migrations — bringing `rots_combat` to 15 TUs total — see caveat below and §10's "combat-pilot wave, step 4 sixth slice" / "combat-trio wave, step 4 seventh slice" / "behavior wave, step 4 ninth slice" / "spell-family closure wave, step 4 eleventh slice" / "spec-pair wave, step 4 twelfth slice" as-built notes. **`mobact` — the row's original sketch listed it here, but it did NOT join `rots_combat`**: the behavior wave's closure check found it homes in `rots_script` instead (see that row below and the §3 REVISION downstream note) — this is this document's first instance of an original-sketch TU resolving into a DIFFERENT row than the one it was originally assigned to.) |
...
**As-built (spell-family closure wave, step 4 eleventh slice):** the migration playbook's own
long-flagged "Ambitious tier" row — the spell-casting registrar `spell_pa.cpp` plus its two closure
partners `mage.cpp`/`ranger.cpp` — promotes into `rots_combat` as **ONE joint membership commit**,
growing it from 12 to **15 TUs**. Combat DEFER drops **5 → 2** (`spec_ass`/`spec_pro` remain, the
last row before the DEFER list reaches 0 under this wave's own §Wave B charter). This is the
playbook's fourth application and the first to actually re-run the `nm` closure check against the
stale combat-peer=76 estimate rather than merely flagging it stale: 44 of the 76 had already
dissolved into TUs `rots_combat` already contained (prior waves' own work), 35 more were intra-family
(spell_pa's registrar dispatching into mage's `spell_*` implementations), leaving 7 genuine app-tier
residual symbols. The joint-commit shape is a true bidirectional file-level cycle — 35
`spell_pa → mage` dispatch edges against 1 `mage → spell_pa` back-edge (`new_saves_spell`) — the
same class as the combat-pilot wave's `clerics ↔ fight` cycle but with a starkly lopsided edge
count; `ranger.cpp`'s own edge into the family (`say_spell`, into spell_pa) is one-directional, not
a cycle, but landed in the same commit regardless. **`show_tracks`** (`ranger.cpp`'s call into
`graph.cpp`, `rots_pathfind`, L4) was this wave's candidate for a third permanent L3→L4 inversion,
after `limits → mobact` (behavior wave) and `script.cpp`'s `call_trigger` (Cluster B wave); the
body-read found it is presentation over L3-reachable world/room data with zero graph-adjacency/BFS
state, so Default A (RELOCATE) fired instead — the function and its two helpers
(`track_desc`/`water_track_desc`) moved verbatim into `db_world.cpp` (`rots_world`), and the
codebase's inversion count stays at two, not three. One genuine census finding not in the design
spec's own default list: ranger's `on_character_returned` edge into `big_brother.cpp` had no
existing hook, becoming a fourth `entity_hooks.h` big_brother sibling (joining
`on_character_died`/`on_character_afked`/`on_corpse_decayed`). The rider gate stayed untouched at 2
of ≤3 (zero spec-proc-dispatcher edges found in any of the three TUs); zero new `combat_command`
cells (every `do_*` up-call mapped onto one of the existing 29). Test count: 1468 → 1487 (Task 1
+17 seam/hook/coverage-rider tests, Task 2 +0 — a verified-zero discriminator gap plus a
controller-caught fix converting spell_pa's `is_target_valid` onto the entity hook ahead of the
joint commit, Task 3 +2 the `look` cell's first-caller discriminator pair, Task 4 +0 a pure
CMakeLists.txt move). `CombatLayerAcyclicity` went green on the first build attempt, both hosts, no
bisection needed. See `docs/BUILD.md`'s "The spell-family closure wave" subsection and
`docs/superpowers/combat-migration-playbook.md`'s "The spell-family closure wave" section for the
full seam/relocation/rider-gate account.

**As-built (spec-pair wave, step 4 twelfth slice — the row's final slice):** the playbook's last
two DEFER rows — `spec_pro.cpp` (the spec-proc command-driver TU) and `spec_ass.cpp` (its SPECIAL
assignment registrar) — promote in **two SEQUENTIAL membership commits** (spec_pro first, spec_ass
second; `nm` found the pair's dependency strictly one-directional, `spec_ass → spec_pro` = 39
edges, `spec_pro → spec_ass` = 0, so no cycle forced a joint commit). Both land **not** in
`rots_combat` but in `rots_script`, growing it from 5 to **7 TUs** — the mobact "driver homes with
the engine it invokes" ruling (behavior wave) extended one link further: the spec-proc bodies and
their assigner both home with the `virt_*` dispatch machinery that invokes them. T0's tier census
found `rots_combat` irreducible on three upward edges (`spec_pro`'s `find_first_step`/
`command_interpreter` hook, `spec_ass`'s `set_virt_program_number_hook`/`set_virt_assignmob_hook`,
all resolving to `rots_pathfind`/`rots_script`) that `rots_script` resolves downward/intra-lib
instead — the same closure-check method the behavior wave used to settle `mobact`'s tier. This wave
also lands the playbook's first genuinely new seam taxonomy since the L4-band's own inception: a
**registrar-lookup family** (`enum class registered_special { gen_board, postmaster, receptionist
}` + `lookup_registered_special()`/`set_registered_special()`, `script_hooks.h`, abort-tripwire
default on unregistered lookup) for spec_ass's three pure address-of fn-ptr references into
app-tier subsystems (`boards.cpp`/`mail.cpp`/`objsave.cpp`) — confirmed exactly 3 edges by `nm`,
pre-authorized, no 4th, no auto-STOP. Two more `script_hooks.h` hooks (`command_min_position`,
`target_check`) are SAFE-SENTINEL accessors, not `void*` dispatchers, so the rider gate stays
untouched at 2 of ≤3. Zero new `combat_command` cells (all 14 real `do_*` up-calls map onto
existing cells). Test count: 1487 → 1510 (Task 1 +11 seam/hook/relocation tests, Task 2 +12
first-caller discriminator pairs, Task 3 +0 a verified-zero discriminator gap, Task 4 +0 two pure
CMakeLists.txt SEQUENTIAL moves). `ScriptLayerAcyclicity` went green on the first build attempt at
both commits, both hosts, no bisection needed. **Combat DEFER reaches 0 — the combat row is
CLOSED.** See `docs/BUILD.md`'s "The spec-pair wave" subsection and
`docs/superpowers/combat-migration-playbook.md`'s "The spec-pair wave" and "THE COMBAT ROW IS
CLOSED" sections for the full seam/tier-ruling/five-wave-arc account.
