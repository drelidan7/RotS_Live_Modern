...
## 6. As-built record (implementation deviations, adjudicated)

Two deviations from this spec's letter landed during implementation, both reviewer-adjudicated:

- **§1.3's Table-A row floor**: the brief's literal floor 9 made the sabotage-(b) self-test direction unreachable (the row drop tripped the floor's SystemExit before Direction 2 ran). First implementation lowered the floor to 8 — rejected in task review as reopening the single-row-deletion blind spot (verified live: deleting the `shop_data::in_room` row, whose coverage tokens duplicate `obj_data::in_room`'s, or the coverage-less `was_in_room` row passed with zero errors at floor 8). As built: floor stays 9 (== the real Table A row count, zero headroom), and the synthetic self-test fixture carries the headroom instead (10 rows, one UNTRACKED-BY-DESIGN synthetic-only row).
- **§2.3's "the GET_LOADROOM block is UNCHANGED / persisted-NOWHERE survives" claim was empirically false**: once the clamp genuinely places a bugged character, `save_char(ch, NOWHERE, 0)` takes its FIRST fail-safe arm (`db_players.cpp:1951` — `load_room == NOWHERE && location_of(ch) != NOWHERE` persists the location's vnum), which structurally precedes the channel arm this spec reasoned about. As built, both flipped tests assert `GET_LOADROOM == room_vnum_for(kDistinctStartRnum)`. Behaviorally equivalent on next login (a persisted racial-start vnum and a persisted NOWHERE both route to the racial start room), and the pre-existing arm applies to every placed character — it is not new behavior from the clamp. Consequence: `PostLoginSaveOfABuggedCharacterPersistsNowhereThroughTheRealLoadCharacter` is no longer the channel-retirement red-first witness (its name is now a partial misnomer — deferred-minor rename candidate); the discriminator's standing witness is `LoadCharacterRetiresTheVnumChannelOnceThePlacementIsDone`, verified failing (35 vs −1) under the retirement-revert sabotage. **W-3 (final whole-branch review, this PR)**: the misnomer is now closed — the test was renamed to `PostLoginSaveOfABuggedCharacterPersistsThePlacedRoomThroughTheRealLoadCharacter`, matching what it actually asserts (the placed room's vnum, not NOWHERE); every tracked reference to the old name was updated to match, except the LS-3b program spec (`docs/superpowers/specs/2026-07-23-locationsystem-program-design.md`), which stays as the historical as-built record per reviewer ruling W-4.
