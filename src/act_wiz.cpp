...
void do_stat_room(struct char_data* ch)
{
    ...
    send_to_char(
        std::format("Room name: {}{}{}\n\r", CC_USE(ch, COLOR_ROOM), rm->name, CC_NORM(ch)),
        ch);
...
void do_stat_object(struct char_data* ch, struct obj_data* j)
{
    ...
    case ITEM_SCROLL:
    case ITEM_POTION:
        type_specific_line = std::format("Spells: {}, {}, {}, {}", j->obj_flags.value[0], j->obj_flags.value[1],
            j->obj_flags.value[2], j->obj_flags.value[3]);
        break;
    case ITEM_WAND:
    case ITEM_STAFF:
        type_specific_line = std::format("Spell: {}, Stamina: {}", j->obj_flags.value[0], j->obj_flags.value[1]);
        break;
    ...
    case ITEM_MISSILE:
        ...
        break;
    case ITEM_ARMOR:
        type_specific_line = std::format("Absorb: [{}]%, Min.abs.: [{}], Encum: [{}], dodge[{}]", armor_absorb(j),
            j->obj_flags.value[1], j->obj_flags.value[2], j->obj_flags.value[3]);
        break;
    case ITEM_TRAP:
        type_specific_line = std::format("Spell: {}, - Hitpoints: {}", j->obj_flags.value[0], j->obj_flags.value[1]);
        break;
    case ITEM_CONTAINER:
        type_specific_line = std::format("Max-contains: {}, Locktype: {}, Corpse: {}", j->obj_flags.value[0],
    ...
    send_to_char(type_specific_line, ch);
...
void do_stat_character(struct char_data* ch, struct char_data* k)
{
    ...
    std::string pos_line = std::format("Pos: {}, Fighting: {}", static_cast<const char*>(buf2),
...
ACMD(do_zone)
{
    ...
    send_to_char(std::format("Zone #{}: {}\n\r", zonnum, zone_table[tmp].name), ch);
...
ACMD(do_dc)
{
    ...
    log(std::format("(GC) Connection closed by {}.", GET_NAME(ch)));
}
...
ACMD(do_wizutil)
{
    ...
    case SCMD_REROLL:
        ...
        log(std::format("(GC) {} has rerolled {}.", GET_NAME(ch), GET_NAME(vict)));
    ...
    case SCMD_RETIRE:
        ...
        log(std::format("(GC) {} has retired {}.", GET_NAME(ch), GET_NAME(vict)));
        break;
    case SCMD_REACTV:
        if (!PLR_FLAGGED(vict, PLR_RETIRED)) {
        ...
        log(std::format("(GC) {} has reactivated {}.", GET_NAME(ch), GET_NAME(vict)));
        break;
    case SCMD_REHASH:
    ...
    case SCMD_NOTE:
        ...
        log(std::format("(GC) {} wrote a note about {}.", GET_NAME(ch), GET_NAME(vict)));
        send_to_char("OK.", ch);
        break;
    }
...
ACMD(do_account)
{
    ...
    if (!str_cmp(subcommand, "show")) {
        ...
        send_to_char(account::format_account_summary(account_data), ch);
        return;
    }
