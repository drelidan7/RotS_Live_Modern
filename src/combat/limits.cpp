...
void point_update(void)
{
    ...
    for (i = character_list; i; i = next_dude) {
        ...
        if (GET_POS(i) >= POSITION_STUNNED) {
            ...
            // TASK-021: this tick still ENGAGES the poisoned character with
            // itself (nobody else is in the fight), but the kill is credited to
            ...
            if (movechance < 75) {
                ...
                if (!(room->dir_option[direction])) {
                    ...
                    if (!(room_affected_by_spell(room_by_id_total(roomnum), SPELL_MIST_OF_BAAZUNGA))) {
                        ...
                        affect_to_room(room_by_id_total(roomnum), &newaf, moved_caster);
                        affect_remove_room(room, tmpaf);
                        // affect_remove_room() ends in put_to_affected_type_pool(),
                        // which is `free(oldaf)` in every build
                        // (entity_lifecycle.cpp) -- so `tmpaf` is dangling from here
                        // on. The `if (tmpaf)` test below was already written as if
                        // something nulled it after a move; nothing ever did, and it
                        // read (and could re-remove) freed storage on every mist that
                        // drifted. TASK-021 supplies the missing null; the loop
                        // increment re-reads `next_tmpaf`, saved at the top.
                        tmpaf = nullptr;
                    }
                }
            }
