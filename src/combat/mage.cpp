...
ASPELL(spell_earthquake)
{
    ...
    if (crack_chance) {
        ...
        const auto fall = [&](char_data* faller, bool landing_saved) {
            ...
            stop_riding(faller);
            rots::entity::dispatch_char_from_room(faller);
        ...
        // extract_char(), which free_char()s an NPC caster (or re-places a
        // player), and this loop keeps using `caster` for every later occupant
        // -- so the caster's fall must be the spell's final act (the TASK-018
        // fireball shape).
