...
// TASK-026: the set of characters that took part in one death, built by
// kill_contributors() (fight.cpp) and carried into pkill_create() below.
//
// It exists because pkill.cpp's three record-building walks used to derive
// their own answer straight from `combat_list`, and a walk of the combat list
// structurally cannot see two kinds of contributor this branch now credits:
// the character recorded as the victim's poisoner (who may be nowhere near the
// fight, and may have stopped fighting long ago), and the caster a room affect
// names when its tick lands the killing blow from another room entirely. One
// list is built once, at the death, and the three walks iterate it instead --
// so weight, opponent count and the records themselves can never disagree
// about who was involved.
//
// Fixed capacity and no heap: this is built on the death path, is consumed
// immediately, and never outlives the call. A death with more than kCapacity
// distinct contributors drops the surplus (and says so in the log) rather than
// allocating.
...
// told did it -- it may be null (nobody is credited), and it may be standing
// anywhere. Membership rules, applied to every candidate: an NPC pet or
// orc-friend whose master stands in the same room contributes as its MASTER
// (damage_credited()'s own redirect); the victim itself and immortals are
// never contributors. Room-affect casters get no participation memory beyond
// this one death -- only the tick that actually kills names its caster here.
