 ...
 */
// report_wrong_target() relocated verbatim to visibility.cpp (spell-family
// closure wave Task 1; sf-census.md section 4.1: RELOCATE-CLEAN --
// send_to_char()/IS_SET(mask,...) bitmask checks only, no visibility-family
// dependency of its own). Declaration unchanged (interpre.h has none; every
// caller uses a local extern, unaffected by this move).
...
// target_from_word() relocated verbatim to visibility.cpp (spell-family
// closure wave Task 1; sf-census.md section 4.1: RELOCATE-CLEAN -- its
// deps (get_char_room_vis()/get_char_vis(), already this file's own peers;
// get_obj_in_list()/get_obj(), rots_entity/L2; get_from_txt_block_pool(),
// output_seam/L1; libc string scanning) all resolve downward or intra-lib
// once spell_pa.cpp promotes). Declaration unchanged (interpre.h has none;
// every caller uses a local extern, unaffected by this move).
...
void assign_command_pointers(void)
{
    ...
    COMMANDO(249, POSITION_DEAD, do_convert_plrobjs, LEVEL_IMPL, FALSE, 0,
        FULL_TARGET, FULL_TARGET, 0);
    COMMANDO(250, POSITION_DEAD, do_convert_exploits, LEVEL_IMPL, FALSE, 0,
        FULL_TARGET, FULL_TARGET, 0);
