...
// Promoted to external linkage in obj_files.cpp (persist-split PS Task 2):
// Crash_load()/Crash_listrent() (below) still call all three cross-TU.
...
// P inventory (staging map, path helpers, JSON writer, Crash_get_filename/Crash_delete_file/Crash_delete_crashfile/Crash_clean_file/update_obj_file, register_char_teardown_hook) relocated to obj_files.cpp (persist-split PS Task 2).
...
// Crash_is_unrentable relocated to obj_files.cpp (persist-split PS Task 2).
...
// cost_per_day relocated to obj_files.cpp (persist-split PS Task 2).

// secs_to_unretire (+ its RENT_TIME macro) relocated to obj_files.cpp (persist-split PS Task 2).
