...
SPECIAL(mob_cleric)
{
    ...
    wtl_base.flg = 1;
    do_cast(host, mutable_arg(""), &wtl_base, CMD_CAST, 0);
    return FALSE;
}
...
SPECIAL(mob_ranger)
{
    ...
    if (tmpch) {
        ...
        do_ambush(host, mutable_arg(""), &tmpwtl, CMD_AMBUSH, 0);
...
SPECIAL(resetter)
{
    ...
    if (!str_cmp_nullable(wtl->targ2.ptr.text->text, "pracreset")) {
        ...
        do_pracreset(host, mutable_arg(""), &wtltmp, CMD_PRACRESET, 0);
        ...
        if (tmpno) { // 33% she will fly off, 33% attack, 33% kidnap
            ...
            if (victim && GET_LEVEL(victim) < LEVEL_GOD) {
                if (tmpno == 1) {
                    ...
                    // TASK-021: the huntress owns this poison, so a captive
                    // who dies of it in the cells died to a MOB -- without the
                    // record the death would credit nobody at all.
