...
// apply_spell_damage()'s two forms (TASK-021). The live form scales the damage
// by the victim's saving throw against the LIVE caster and credits that caster
// with the kill. The credited form takes a cast-time caster_snapshot for the
// scaling and a separate credited_killer for the kill, so a room affect that
// ticks after its caster changed, left the room, or left the game still scales
// from the cast and still credits the right character (or nobody). Both run
// the identical multiplier -- one shared helper in mage.cpp owns it.
...
// The saving throw the two forms above scale by, in the same two shapes.
