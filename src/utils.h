...
// max_race_prof_level() lives in rots/core/character.h, beside the PROF_*/
// RACE_* constants it reads; this macro is the char_data-shaped face of it
    ...
    do {                                                                                     \
        ...
        if (ch->delay.wait_value != 0) {                                                     \
            if (prir >= ch->delay.priority) {                                                \
                ch->delay.subcmd = -1;                                                       \
                complete_delay(ch);                                                          \
                /* Completion may have queued another action; keep its targets and links. */ \
                if (ch->delay.wait_value != 0) {                                             \
